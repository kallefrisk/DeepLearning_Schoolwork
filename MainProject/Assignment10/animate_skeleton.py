...
def create_skeleton_animation(df, output_file='skeleton_animation.gif'):
...
def plot_multiple_frames(df, frame_indices=None, output_file='skeleton_multiframe.png'):
...
def main():
    ...
    df = pd.read_csv("DeepLearning_Schoolwork/MainProject/Assignment9/data/kinect_good_preprocessed/A1_kinect.csv")
    ...
    plot_multiple_frames(df, frame_indices=[0, 1, 2, 3], output_file='skeleton_frames.png')
    
    # Option 2: Create animated visualization
    print("\nCreating animation...")
    try:
        anim = create_skeleton_animation(df, output_file='skeleton_animation.gif')
